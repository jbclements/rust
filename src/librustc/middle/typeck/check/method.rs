...
impl<'self> LookupContext<'self> {
    ...
    pub fn push_inherent_candidates_from_trait(&self,
        ...
        let index = match ms.iter().position_(|m| m.ident.name == self.m_name) {
            Some(i) => i,
    ...
    pub fn push_inherent_candidates_from_self(&self,
        ...
        match methods.iter().position_(|m| m.ident.name == self.m_name) {
            Some(i) => {
        ...
        if method_info.is_none() {
            for ty::trait_supertraits(tcx, did).iter().advance |trait_ref| {
                ...
                match supertrait_methods.iter().position_(|m| m.ident.name == self.m_name) {
                    Some(i) => {
                        method_info = Some(MethodInfo {
    ...
    pub fn push_candidates_from_impl(&self,
        ...
        let idx = {
            match impl_info.methods.iter().position_(|m| m.ident.name == self.m_name) {
                Some(idx) => idx,
                None => { return; } // No method with the right name.
            }
        };
