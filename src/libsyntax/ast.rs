...
pub enum Pat_ {
    ...
    // HIDDEN INVARIANT: neither of these next two forms can
    // contain expressions that contain variable references.
    // This is required by our current hygiene implementation,
    // and enforced by the parser; if we had an official grammar,
    // that would be a great place to document this. If we decide
    // to change this, we'd need to revisit hygiene for patterns
    // and perform renaming on bindings in patterns more carefully.
    PatLit(Gc<Expr>),
