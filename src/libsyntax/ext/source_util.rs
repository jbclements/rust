...
// include_macros! : parse the given file as an expr, which must
// produce a block containing macro defs.
// This is generally a bad idea because it's going to behave
// unhygienically.
pub fn expand_include_macros(cx: ext_ctxt, sp: span, tts: ~[ast::token_tree])
    -> base::MacResult {
    let file = get_single_str_from_tts(cx, sp, tts, "include_macros!");
    let p = parse::new_sub_parser_from_file(
        cx.parse_sess(), cx.cfg(),
        &res_rel_file(cx, sp, &Path(file)), sp);
    // must be a block containing macro defs (which will look like
    // uses of macro_rules!
    base::MRMacroDefs(p.parse_expr())
}
