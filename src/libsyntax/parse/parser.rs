...
pub impl Parser {
    ...
    fn parse_block_tail_(&self, lo: BytePos, s: blk_check_mode,
        ...
        // the first time through, we need to add these attributes:
        let mut attributes_use_once = attrs_remaining;

        while *self.token != token::RBRACE {
            // parsing items even when they're not allowed lets us give
            // better error messages and recover more gracefully.
            let initial_attrs = vec::append(attributes_use_once,
                                            self.parse_outer_attributes());
            // blecch:
            attributes_use_once = ~[];
            match *self.token {
                token::SEMI => {
                    if !vec::is_empty(initial_attrs) {
